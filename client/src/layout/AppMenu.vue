...
onMounted(async () => {
  ...
  for (let i = 0; i < servers.length; i++) {
    if ('' == servers[i].serverName) {
      servers[i].serverName = servers[i].id.slice(0, 6) + ' - waiting for data';
    }
    localmenuList[1].items.push({ label: servers[i].serverName, icon: 'pi pi-fw pi-map', to: '/map?server=' + servers[i].id });
  }

  // console.log('menulist in onMounted', localmenuList);
  model.value = localmenuList;
});
// @request.auth.email:lower = owner
// id = @request.auth.id
